...
# Kotak columns: Dt | Description | Chq. No. | Dr | Cr | Balance
_HEADER_KEYWORDS = ["description", "dr", "cr", "balance"]


class KotakParser(BaseParser):
    """Parser for Kotak Mahindra Bank account statements."""
        ...
        for page in pdf.pages:
            for table in page.extract_tables():
                ...
                if header_idx is None:
                    continue
                col_map = _detect_kotak_columns(table[header_idx])
                rows.extend(_parse_kotak_table(table, header_idx, col_map, warnings))
        return rows
    ...
    def _extract_metadata(
        ...
        opening = _kotak_balance(text, ["opening balance"])
        closing = _kotak_balance(text, ["closing balance"])
        period = _kotak_period(text)
        return opening, closing, period


def _detect_kotak_columns(header_row: list[str | None]) -> dict[str, int]:
    mapping: dict[str, int] = {}
    for idx, cell in enumerate(header_row):
        c = str(cell or "").strip().lower()
        if c in ("dt", "date") or ("date" in c and "value" not in c):
        ...
        elif "balance" in c or c == "bal":
            mapping.setdefault("balance", idx)
    return mapping


def _parse_kotak_table(
...
def _kotak_balance(text: str, labels: list[str]) -> str | None:
    lower = text.lower()
    for label in labels:
...
def _kotak_period(text: str) -> StatementPeriod | None:
    m = re.search(
