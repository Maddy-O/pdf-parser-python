...
Format: Date | Description | Ref. No | Value Date | Withdrawal (Dr) | Deposit (Cr) | Balance
...
_HEADER_KEYWORDS = ["date", "withdrawal", "deposit", "balance"]
...
class IciciParser(BaseParser):
    ...
    def _extract_transactions(self, pdf: pdfplumber.PDF, warnings: list[str]) -> list[TransactionRow]:
        ...
        for page in pdf.pages:
            for table in page.extract_tables():
                ...
                if h is None:
                    continue
                col_map = _detect_icici_cols(table[h])
    ...
    def _extract_metadata(self, pdf):
        ...
        meta.account_number_masked = _icici_account(text)
        meta.account_holder = _icici_holder(text)
        meta.period = _icici_period(text)
        return opening, closing, meta


def _detect_icici_cols(header_row):
    m = {}
    for i, c in enumerate(header_row):
        ...
        elif any(k in cell for k in ["description", "particulars", "narration", "remarks"]):
            m.setdefault("desc", i)
        elif "ref" in cell:
            m.setdefault("ref", i)
        elif "withdrawal" in cell or "debit" in cell or " dr" in cell:
            m.setdefault("debit", i)
        elif "deposit" in cell or "credit" in cell or " cr" in cell:
...
def _icici_account(text):
    m = re.search(r"account\s+(?:no|number)[.:\s]*(\d{6,16})", text, re.IGNORECASE)
    if m:
        num = m.group(1)
        return f"****{num[-4:]}"
    return None
...
def _icici_holder(text):
    m = re.search(r"(?:account\s+holder|customer\s+name)[:\s]+([A-Z][A-Z\s]{2,40})", text, re.IGNORECASE)
    return m.group(1).strip().title() if m else None


def _icici_period(text):
    m = re.search(r"(?:from|statement period)[:\s]+(\d{2}/\d{2}/\d{4})\s+(?:to|–|-)\s+(\d{2}/\d{2}/\d{4})", text, re.IGNORECASE)
    if m:
