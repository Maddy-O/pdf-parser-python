...
class FabParser(BaseParser):
    BANK_CODE = BankCode.FAB
    ...
    def _extract_transactions(self, pdf: pdfplumber.PDF, warnings: list[str]) -> list[TransactionRow]:
        ...
        for page in pdf.pages:
            for table in page.extract_tables():
                ...
                if h is None:
                    continue
                col_map = _detect_fab_cols(table[h])
    ...
    def _extract_metadata(self, pdf):
        ...
        meta.account_number_masked = _fab_account(text)
        meta.period = _fab_period(text)
        return opening, closing, meta


def _detect_fab_cols(header_row):
    m = {}
    for i, c in enumerate(header_row):
        cell = str(c or "").strip().lower()
        if "date" in cell and "value" not in cell:
            m.setdefault("date", i)
        elif any(k in cell for k in ["description", "narration", "details"]):
            m.setdefault("desc", i)
        elif "ref" in cell:
            m.setdefault("ref", i)
...
def _fab_period(text):
    m = re.search(r"(?:from|period|statement date)[:\s]+(\d{2}[/-]\d{2}[/-]\d{4})\s+(?:to|–|-)\s+(\d{2}[/-]\d{2}[/-]\d{4})", text, re.IGNORECASE)
    if m:
