"""
Chase Bank (US) statement parser.

Chase PDF format:
- Bank account: Date | Description | Amount (signed: negative = debit)
- Credit card: sections "Payments and Credits" / "Purchase and Adjustments"
  with columns: Date | Description | Amount
US date format: M/D/YYYY (e.g. 01/15/2025)
"""
import re
import pdfplumber
...
_CC_SECTION_CREDIT = {"PAYMENTS AND CREDITS", "PAYMENTS & CREDITS", "CREDITS AND ADJUSTMENTS"}
_CC_SECTION_DEBIT  = {"PURCHASE AND ADJUSTMENTS", "PURCHASES", "TRANSACTIONS"}


class ChaseParser(BaseParser):
    """Chase checking / savings account statement parser."""
    BANK_CODE = BankCode.CHASE
    ...
    def _extract_transactions(self, pdf: pdfplumber.PDF, warnings: list[str]) -> list[TransactionRow]:
        ...
        for page in pdf.pages:
            for table in page.extract_tables():
                ...
                rows.extend(_parse_signed_table(table, h, col_map, warnings, self.DATE_HINT, Currency.USD))
        return rows
    ...
    def _extract_metadata(self, pdf):
        ...
        period = _chase_period(text)
        meta = self._default_metadata()
        meta.period = period
        meta.account_number_masked = _account_num(text)
...
class ChaseCreditParser(BaseParser):
    ...
    def _extract_transactions(self, pdf: pdfplumber.PDF, warnings: list[str]) -> list[TransactionRow]:
        """
        Chase CC PDFs group transactions under section headers.
        We track the current section to assign DEBIT/CREDIT correctly.
        """
        rows: list[TransactionRow] = []
        current_type = "DEBIT"  # default assumption
        ...
        for page in pdf.pages:
            ...
            for line in text.split("\n"):
                ...
                if any(s in upper for s in _CC_SECTION_CREDIT):
                    current_type = "CREDIT"
                elif any(s in upper for s in _CC_SECTION_DEBIT):
                    current_type = "DEBIT"
            ...
            for table in page.extract_tables():
                ...
                if h is None:
                    continue
                col_map = _detect_cols(table[h])
                rows.extend(_parse_cc_section_table(table, h, col_map, current_type, warnings, Currency.USD))
        return rows
...
def _detect_cols(header_row):
    ...
    for i, c in enumerate(header_row):
        ...
        elif any(k in cell for k in ["description", "details", "memo"]):
            m.setdefault("desc", i)
        elif "amount" in cell or "debit" in cell:
            m.setdefault("amount", i)
    return m


def _parse_signed_table(table, header_idx, col_map, warnings, date_hint, currency):
    """Parse a table where negative amount = debit, positive = credit."""
    rows = []
    for row in table[header_idx + 1:]:
        ...
        date = parse_date(get("date"), date_hint)
        ...
        if not amt: continue
        # Chase: negative = withdrawal/debit, positive = deposit/credit
...
def _parse_cc_section_table(table, header_idx, col_map, forced_type, warnings, currency):
    rows = []
    for row in table[header_idx + 1:]:
        if not row or all(c is None or str(c).strip() == "" for c in row):
            continue
...
def _account_num(text):
    m = re.search(r"account\s+(?:ending|number)[:\s#]*(\d{4})", text, re.IGNORECASE)
